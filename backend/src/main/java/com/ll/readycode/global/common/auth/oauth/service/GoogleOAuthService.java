...
public class GoogleOAuthService implements OAuthService {
  ...
  public Token login(String authCode) {
    ...
    GoogleTokenResponse googleToken = getAccessToken(authCode);
    GoogleUserInfo userInfo = getUserInfo(googleToken.accessToken());
  ...
  /**
   * Google 인가 코드를 통해 Access 토큰을 요청합니다.
   *
   * @param authCode Google 로그인 성공 후 리디렉션으로 전달된 인가 코드
   * @return GoogleTokenResponse Google로부터 받은 Access & Refresh 토큰 정보
   */
  private GoogleTokenResponse getAccessToken(String authCode) {
  ...
  /**
   * Google API를 통해 사용자 정보를 조회합니다.
   *
   * @param accessToken Google에서 발급받은 액세스 토큰
   * @return GoogleUserInfo Google 사용자 프로필 정보
   */
